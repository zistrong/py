...
def part1():
    ...
    """
    with readFile('12') as file:
        visited= []
        matrix = [[x for x in line.strip()] for line in file]
        startX = 0
        startY = 0
        for i in range(len(matrix)):
            if 'S' in matrix[i]:
                startX = i
                startY = matrix[i].index('S')
        print(startX, startY)
...
def visit(i, j, matrix, di = 0, dj = 0):
    if i +di>len(matrix) or j+dj > len(matrix[i]):
        return None
